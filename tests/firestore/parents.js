...
async function parents0000() {
  ...
  const doc0 = did(0);
  const data0 = { fName: 'Angelo', lName: 'Adams' };
  ...
  try {
    ...
    await assertSucceeds(getDoc(user0, doc0).get());
...
async function parents0001() {
  ...
  const doc0 = did(0);
  const doc1 = did(1);
  const data0 = { fName: 'Bobby', lName: 'Black' };

  let passed = true;
  try {
    // Attempt to create a doc as an authenticated user.
    await assertSucceeds(getDoc(user0, doc0).set(data0));
...
async function parents0002() {
  ...
  const admin = createAdminApp(id);
  const user0 = createUserApp(id, new User(uid(0), email));
  const user1 = createUserApp(id, new User(uid(1), email));
  const user2 = createUserApp(id, null);

  const doc0 = uid(0);
  const doc1 = did(0);
  const doc2 = did(1);
  const data0 = { fName: 'Charles', lName: 'Crawfish' };
  const data1 = { fName: 'Charlie' };

  // Load test data.
  await getDoc(admin, doc0).set(data0);
  await getDoc(admin, doc1).set(data0);
  await getDoc(admin, doc2).set(data0);

  let passed = true;
  try {
    // Attempt to update a doc as a properly authenticated user.
    await assertSucceeds(getDoc(user0, doc0).update(data1));
    ...
    await assertFails(getDoc(user2, doc2).update(data1));
  } catch {
    passed = false;
  }
...
async function parents0003() {
  ...
  const doc1 = did(0);
  const doc2 = did(1);
  const data0 = { fName: 'Dante', lName: 'Dunn' };
  ...
  try {
    ...
    await assertSucceeds(getDoc(user0, doc0).delete());
    // Attempt to delete a doc as an improperly authenticated user.
    await assertFails(getDoc(user1, doc1).delete());
    // Attempt to delete a doc as an unauthenticated user.
    await assertFails(getDoc(user2, doc2).delete());
  } catch {
    passed = false;
  }
